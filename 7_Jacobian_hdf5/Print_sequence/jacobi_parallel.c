...
// save matrix to file
void print_hdf5_time_evolution( double * matrix, double * matrix_new, size_t d_loc, size_t dimension, int rank, int npes, int rest, int it );
...
int main(int argc, char* argv[]){
  ...
  for( it = 0; it < iterations; ++it ){
    ...
    if(it % f_dump == 0) {
#ifndef PAR
      print_hdf5_time_evolution( matrix, matrix_new, d_loc, dimension, rank, p, rest, it );
#else
      print_hdf5_time_evolution_par( matrix, d_loc, dimension, rank, p, rest, offset, it );
#endif
    }
  }
				...
				int it )  // iteration   
{
  ...
  if( !rank ) {
	...
	offset = rest;
      ...
      MPI_Recv( matrix_new, ( dimension + 2 ) * ( d_loc + include_lower_border ), MPI_DOUBLE, i, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      ...
      status = H5Sselect_hyperslab ( file_space, H5S_SELECT_SET, start_3d_mem, NULL, dims_data, NULL );
      status = H5Dwrite( dataset_id, H5T_NATIVE_DOUBLE, mem_space, file_space, H5P_DEFAULT, matrix_new );
    }
    // relase the objects to avoid memory leaks
    status = H5Sclose( mem_space );
    status = H5Sclose( file_space );
